...
import { batchFetchVideoStats, fetchVideoStats } from './youtube'
...
export const syncAllFilmStats = async (onProgress = null) => {
  try {
    ...
    if (!films || films.length === 0) {
      return { success: true, synced: 0, message: 'No films to sync' }
    }

    onProgress?.({ stage: 'fetching', total: films.length, done: 0 })
    ...
    return {
      ...
      synced,
      errors,
      message: `Synced ${synced} of ${films.length} films`
    }
