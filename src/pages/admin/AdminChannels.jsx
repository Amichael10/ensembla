...
function PeopleSearch({ value, onChange }) {
  ...
  return (
    <div ref={ref} className="relative">
      <label className="block text-text-muted text-[10px] font-black uppercase tracking-[0.2em] mb-2 px-1">
        Channel Owner
      </label>
      ...
      {value ? (
        <div className="flex items-center gap-3 bg-surface-2 border border-brand/20 rounded-lg px-4 py-3 shadow-inner">
          ...
          <div className="flex-1 min-w-0">
            <span className="text-text-primary text-sm font-black block truncate">{value.name}</span>
            <span className="text-text-muted text-[10px] uppercase font-bold tracking-widest">Linked Identity</span>
          </div>
              ...
              {results.map(p => (
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-text-muted text-[10px] font-bold uppercase tracking-widest">{p.known_for_department || 'Artist'}</p>
                  </div>
                </button>
              ))}
            </div>
    ...
    <div className="fixed inset-0 bg-overlay backdrop-blur-md z-[100] flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-surface border border-border rounded-md w-full max-w-2xl my-auto overflow-hidden shadow-2xl flex flex-col animate-in fade-in zoom-in duration-300">
        <div className="px-10 py-8 border-b border-border flex items-center justify-between">
          <div>
            <p className="text-brand text-[10px] font-black uppercase tracking-[0.3em] mb-1 italic">Source Protocol</p>
            <h2 className="text-2xl font-black text-text-primary tracking-tight">
              {channel ? 'Refine Channel' : 'Register Channel'}
            </h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-10 py-8 space-y-8 custom-scrollbar max-h-[60vh]">
          {error && (
            <div className="bg-red-500/10 border border-red-500/20 text-red-500 text-xs font-bold uppercase tracking-widest px-6 py-4 rounded-lg flex items-center gap-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[
              ...
              <div key={f.name}>
                <label className="block text-text-muted text-[10px] font-black uppercase tracking-widest mb-2 px-1">{f.label}</label>
                <input
                  type={f.type || 'text'}
                  name={f.name}
                  value={form[f.name]}
            ...
            <div>
              <label className="block text-text-muted text-[10px] font-black uppercase tracking-widest mb-2 px-1">Category</label>
              <select
                name="category"
                value={form.category}
                onChange={handleChange}
          ...
          <div>
            <label className="block text-text-muted text-[10px] font-black uppercase tracking-widest mb-2 px-1">Description</label>
            <textarea
              name="description"
          ...
          <label className="flex items-center gap-4 cursor-pointer group w-fit">
            ...
            <span className="text-text-primary text-xs font-bold uppercase tracking-widest">Featured Source</span>
          </label>
        </form>
        ...
        <div className="px-10 py-8 border-t border-border bg-surface-2/30 flex gap-4">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 py-4 rounded-lg border border-border text-text-muted font-black text-[10px] uppercase tracking-[0.2em] hover:bg-surface-3 transition-all"
          >
            Abort
          </button>
          <button
            ...
            disabled={saving}
            className="flex-[2] py-4 rounded-lg bg-brand text-white font-black text-[10px] uppercase tracking-[0.2em] hover:bg-brand-hover active:scale-95 transition-all disabled:opacity-50 shadow-lg shadow-brand/20"
          >
            {saving ? 'Processing...' : channel ? 'Update Protocol' : 'Initialize Protocol'}
          </button>
        </div>
      </div>
    </div>
    ...
    <div className="fixed inset-0 bg-overlay backdrop-blur-md z-[100] flex items-center justify-center p-4">
      <div className="bg-surface border border-border rounded-md w-full max-w-lg max-h-[80vh] flex flex-col shadow-2xl animate-in fade-in zoom-in duration-300">
        <div className="px-8 py-6 border-b border-border flex items-center justify-between">
          <div>
            <p className="text-brand text-[10px] font-black uppercase tracking-widest mb-1">Violation Log</p>
            <h2 className="text-xl font-black text-text-primary">{channel.name}</h2>
          </div>
        ...
        <div className="overflow-y-auto flex-1 p-6 space-y-4 custom-scrollbar">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-text-primary font-black uppercase text-xs tracking-widest">No Active Flags</p>
              <p className="text-text-muted text-[10px] mt-1 italic">This channel is in compliance.</p>
            </div>
          ) : flags.map(flag => (
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <p className="text-text-primary text-xs font-black uppercase tracking-widest">{flag.reason?.replace(/_/g, ' ')}</p>
                  </div>
                  {flag.details && <p className="text-text-muted text-xs leading-relaxed mb-3">{flag.details}</p>}
                  <p className="text-text-muted/60 text-[10px] font-bold uppercase">{new Date(flag.created_at).toLocaleDateString()}</p>
                </div>
                {flag.status === 'pending' ? (
                  <button
                    onClick={() => resolveFlag(flag.id)}
                    className="px-4 py-2 bg-green-500/10 text-green-500 text-[10px] font-black uppercase tracking-widest rounded-md hover:bg-green-500 hover:text-white transition-all border border-green-500/20"
                  >
                    Resolve
                  </button>
                ) : (
                  <span className="px-4 py-2 bg-surface-3 text-text-muted text-[10px] font-black uppercase tracking-widest rounded-md">Resolved</span>
                )}
              </div>
            </div>
          ))}
  ...
  return (
    <div className="p-6 max-w-[1600px] mx-auto pb-24">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-8 mb-12">
        <div>
          <p className="text-brand text-[10px] font-black uppercase tracking-[0.4em] mb-2 italic">Broadcast Infrastructure</p>
          <h1 className="text-4xl font-black text-text-primary tracking-tight mb-2">Content Pulse</h1>
          <p className="text-text-muted text-sm max-w-xl font-medium leading-relaxed">
            Regulate incoming assets from <span className="text-brand font-bold">{stats.total} verified nodes</span>. Processing Nollywood metadata signals across verified networks.
          </p>
        </div>
        <button
        ...
        >
          <span className="relative z-10 flex items-center gap-3 font-black">
             <span className="text-lg">⊕</span> Register New Source
          </span>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        {[
          { label: 'Total Nodes',  val: stats.total,       icon: '📡', color: 'text-brand',      bg: 'bg-brand/10' },
          { label: 'Elite Tier',   val: stats.featured,    icon: '💎', color: 'text-blue-400',  bg: 'bg-blue-400/10' },
          { label: 'Synced Hubs',  val: stats.active,      icon: '🔄', color: 'text-green-400', bg: 'bg-green-400/10' },
          { label: 'Reach Capacity', val: formatViewCount(stats.subscribers), icon: '👥', color: 'text-purple-400', bg: 'bg-purple-400/10' },
        ].map(s => (
          <div key={s.label} className="card-cal p-6 relative overflow-hidden group hover:border-brand/30 transition-all duration-500">
            ...
            <div className="flex items-center gap-4 mb-4 relative z-10">
              ...
              <p className="text-[10px] font-black text-text-muted uppercase tracking-[0.2em] opacity-80">{s.label}</p>
            </div>
            <p className={`text-4xl font-black tracking-tighter ${s.color} relative z-10 group-hover:tracking-normal transition-all`}>{s.val}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-10 items-end">
        <div className="lg:col-span-8">
           <div className="relative group w-full md:w-64 mb-4">
              <select
                ...
                className="w-full h-14 bg-surface border border-border rounded-md px-5 text-text-primary text-xs font-black uppercase tracking-widest focus:border-brand focus:outline-none appearance-none cursor-pointer shadow-2xl group-hover:border-border-hover transition-all"
              >
        ...
        <div className="lg:col-span-4 relative group">
          ...
          {syncResult && (
            <div className="absolute right-0 top-full mt-4 bg-surface-2 border border-brand/20 text-brand text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-md shadow-2xl animate-in fade-in slide-in-from-top-2 z-20 flex items-center gap-3">
      ...
      {loading ? (
        ...
        <div className="card-cal p-32 text-center border-dashed border-border-hover">
           ...
           <h2 className="text-text-primary text-2xl font-black tracking-tight mb-3 uppercase tracking-widest">No Sources Detected</h2>
           <p className="text-text-muted max-w-md mx-auto text-sm leading-relaxed mb-10">Adjust your frequency filters or register a new content hub to begin processing metadata.</p>
           <button onClick={() => { setSearch(''); setFilterTab('all'); }} className="px-8 py-4 bg-brand/10 border border-brand/20 text-brand text-[10px] font-black uppercase tracking-widest rounded-lg hover:bg-brand hover:text-white transition-all">Clear Filters</button>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
              ...
              <div className="px-8 pb-10 relative mt-[-3rem]">
                <div className="relative mb-4 group/avatar inline-block">
                  {ch.thumbnail_url ? (
                    ...
                    <div className="w-24 h-24 rounded-md bg-surface-2 border-[6px] border-surface flex items-center justify-center text-3xl font-black text-brand shadow-2xl group-hover:scale-105 transition-all">
                      {ch.name?.charAt(0)}
                    </div>
                  )}
                  <div className="absolute -bottom-2 -right-2 bg-surface border border-border text-text-primary text-[9px] font-black uppercase tracking-widest px-3 py-1.5 rounded-md shadow-2xl">
                ...
                <div className="mb-6">
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <h3 className="text-text-primary text-xl font-black tracking-tight truncate group-hover:text-brand transition-colors">{ch.name}</h3>
                      <p className="text-text-muted text-xs font-bold uppercase tracking-widest mt-1">@{ch.channel_handle?.replace(/^@/, '') || 'frequency'}</p>
                    </div>
                    {ch.category && (
                      <span className="flex-shrink-0 px-3 py-1 bg-surface-2 border border-border text-brand text-[8px] font-black uppercase tracking-widest rounded-lg h-fit mt-1">
                        {ch.category}
                      </span>
                    )}
                  </div>
